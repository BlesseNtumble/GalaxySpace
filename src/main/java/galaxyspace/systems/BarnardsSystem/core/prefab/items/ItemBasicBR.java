...
public class ItemBasicBR extends Item implements ISortableItem {
    ...
    public void getSubItems(CreativeTabs tab, NonNullList<ItemStack> list) {
        if (tab == GSCreativeTabs.GSItemsTab || tab == CreativeTabs.SEARCH) {
            for (int i = 0; i < this.names.length; i++) {
                if (!this.names[i].equals("null")) list.add(new ItemStack(this, 1, i));
            }
        }
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        if (raytraceresult == null) return new ActionResult<ItemStack>(EnumActionResult.PASS, player.getHeldItem(hand));
        ...
        if (itemstack.getItemDamage() == 0) {
            if (raytraceresult.typeOfHit == RayTraceResult.Type.BLOCK) {
                ...
                if (hasWater && world.isAirBlock(blockpos1) && (iblockstate == BRBlocks.BARNARDA_C_GRASS.getDefaultState().withProperty(Barnarda_C_Grass.BASIC_TYPE, Barnarda_C_Grass.EnumBlockGrass.GRASS) || iblockstate == BRBlocks.BARNARDA_C_BLOCKS.getDefaultState().withProperty(Barnarda_C_Blocks.BASIC_TYPE, Barnarda_C_Blocks.EnumBlockBarnardaC.DIRT))) {
