...
public class BarnardsSystemBodies implements IBodies {
	...
	public void registerRender() {
		...
		for (EnumBlockGlowDandelions blockBasic : EnumBlockGlowDandelions.values())
			ClientUtil.registerBlockJson(GalaxySpace.TEXTURE_PREFIX, BRBlocks.BARNARDA_C_GLOW_DANDELIONS, blockBasic.getMeta(), "barnarda/" + blockBasic.getName());
		...
		ClientUtil.registerBlockJson(GalaxySpace.TEXTURE_PREFIX + "barnarda/",  BRBlocks.BARNARDA_C_VIOLET_TRAPDOOR);
		...
		ClientUtil.registerItemJson(GalaxySpace.TEXTURE_PREFIX, BRItems.VIOLET_DOOR, 0,"barnarda/" + BRItems.VIOLET_DOOR.getTranslationKey().substring(5));
	...
	public void registerVariant() {
		...
		ClientProxy.addVariant("barnarda_c_violet_door", "barnarda/", "barnarda_c_violet_door");
		ClientProxy.addVariant("barnarda_c_violet_trapdoor", "barnarda/", "barnarda_c_violet_trapdoor");
		...
		blocks = new String[EnumBlockGlowDandelions.values().length];
		for(int i = 0; i < blocks.length; i++)
			blocks[i] = EnumBlockGlowDandelions.byMetadata(i).getName();

		ClientProxy.addVariant("barnarda_c_glow_dandelions", "barnarda/", blocks);
		...
		ModelLoader.setCustomStateMapper(BRBlocks.BARNARDA_C_VIOLET_DOOR, new StateMap.Builder().ignore(BlockDoor.POWERED).build());
	}
