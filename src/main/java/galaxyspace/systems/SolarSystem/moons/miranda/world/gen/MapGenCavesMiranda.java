...
public class MapGenCavesMiranda
extends MapGenBaseMeta {
    ...
    protected void generateCaveNode(long par1, int par3, int par4, Block[] blockIdArray, byte[] metaArray, double par6, double par8, double par10, float par12, float par13, float par14, int par15, int par16, double par17) {
        ...
        while (par15 < par16) {
            double d6 = 1.5 + (double)(MathHelper.sin((float)((float)par15 * (float)Math.PI / (float)par16)) * par12 * 1.0f);
            double d7 = d6 * par17;
            float f5 = MathHelper.cos((float)par14);
            float f6 = MathHelper.sin((float)par14);
            par6 += (double)(MathHelper.cos((float)par13) * f5);
            par8 += (double)f6;
            par10 += (double)(MathHelper.sin((float)par13) * f5);
            ...
            if (flag || random.nextInt(4) != 0) {
                ...
                if (par6 >= d4 - 16.0 - d6 * 2.0 && par10 >= d5 - 16.0 - d6 * 2.0 && par6 <= d4 + 16.0 + d6 * 2.0 && par10 <= d5 + 16.0 + d6 * 2.0) {
                    int l1 = MathHelper.floor_double((double)(par6 - d6)) - par3 * 16 - 1;
                    int i2 = MathHelper.floor_double((double)(par6 + d6)) - par3 * 16 + 1;
                    int j2 = MathHelper.floor_double((double)(par8 - d7)) - 1;
                    int k2 = MathHelper.floor_double((double)(par8 + d7)) + 1;
                    int l2 = MathHelper.floor_double((double)(par10 - d6)) - par4 * 16 - 1;
                    int i3 = MathHelper.floor_double((double)(par10 + d6)) - par4 * 16 + 1;
                    if (l1 < 0) {
                        l1 = 0;
                    }
                    if (i2 > 16) {
                    ...
                    for (int localY = j2; localY < k2; ++localY) {
                        ...
                        for (int localX = l1; localX < i2; ++localX) {
                            ...
                            for (int localZ = l2; localZ < i3; ++localZ) {
                                ...
                                if (blockIdArray[coords] == GSBlocks.MirandaBlocks && metaArray[coords] == 0 && random.nextInt(50) == 0) {
                                    blockIdArray[coords] = Blocks.air;
                                    continue;
                                }
                                if (blockIdArray[coords] == GSBlocks.MirandaBlocks && metaArray[coords] == 1) {
                                    blockIdArray[coords] = Blocks.air;
                                    continue;
                                }
                                if (blockIdArray[coords] != GSBlocks.MirandaBlocks || metaArray[coords] != 2) continue;
                                blockIdArray[coords] = Blocks.air;
                            }
                        }
                    }
