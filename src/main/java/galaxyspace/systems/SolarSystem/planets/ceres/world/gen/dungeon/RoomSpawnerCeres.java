...
public class RoomSpawnerCeres extends GSDungeonRoom
{
    int sizeX;
    int sizeY;
    int sizeZ;
    ...
    public RoomSpawnerCeres(GSMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
    {
    ...
    {
        return new RoomSpawnerCeres(dungeon, x, y, z, dir);
    }
    ...
    protected void handleTileEntities(Random rand)
    {
        for (final ChunkCoordinates spawnerCoords : this.spawners)
        {
            if (this.worldObj.getBlock(spawnerCoords.posX, spawnerCoords.posY, spawnerCoords.posZ) == Blocks.mob_spawner)
            {
                ...
                if (spawner != null)
                {
                    spawner.func_145881_a().setEntityName(RoomSpawnerCeres.getMob(rand));
                }
            }
        }
    }
