...
public class RoomTreasureCeres extends GSDungeonRoom
{
    int sizeX;
    int sizeY;
    int sizeZ;
    ...
    public RoomTreasureCeres(GSMapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
    {
    ...
    public void generate(Block[] chunk, byte[] meta, int cx, int cz)
    {
            ...
            for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
            {
                for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            this.placeBlock(chunk, meta, i, j, k, cx, cz, GSBlocks.DungeonGlowstones, 0);
                        }
                        else
                        {
        ...
        if (this.placeBlock(chunk, meta, hx, this.posY, hz, cx, cz, GSBlocks.CeresTChestT4, 0))
        {
    ...
    {
        return new RoomTreasureCeres(dungeon, x, y, z, dir);
    }
    ...
    protected void handleTileEntities(Random rand)
    {
        if (!this.chests.isEmpty())
        {
            ...
            {
                this.worldObj.setBlock(coords.posX, coords.posY, coords.posZ, GSBlocks.CeresTChestT4, 1, 0);
                this.worldObj.setTileEntity(coords.posX, coords.posY, coords.posZ, new TileEntityCeresTreasureChest());
