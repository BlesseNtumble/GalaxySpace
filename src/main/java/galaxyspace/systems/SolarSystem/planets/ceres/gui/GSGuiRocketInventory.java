package galaxyspace.systems.SolarSystem.planets.ceres.gui;
...
public class GSGuiRocketInventory extends GuiContainerGC
{
    ...
    static
    {
        for (int i = 0; i < 4; i++)
        {
            GSGuiRocketInventory.rocketTextures[i] = new ResourceLocation(GalacticraftCore.ASSET_PREFIX, "textures/gui/rocket_" + i * 18 + ".png");
        }
    }
    ...
    public GSGuiRocketInventory(IInventory par1IInventory, IInventory par2IInventory, EnumRocketType rocketType)
    {
    ...
    {
        this.mc.getTextureManager().bindTexture(GSGuiRocketInventory.rocketTextures[(this.rocketType.getInventorySpace() - 2) / 18]);
