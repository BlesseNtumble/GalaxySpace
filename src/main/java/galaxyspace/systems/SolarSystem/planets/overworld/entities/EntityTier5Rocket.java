...
public class EntityTier5Rocket extends EntityTieredRocket
{
    ...
    public void onTeleport(EntityPlayerMP player)
    {
        ...
        if (playerBase != null)
        {
            ...
            stats.rocketItem = GSItems.Tier5Rocket;
            stats.fuelLevel = this.fuelTank.getFluidAmount();
            
            int countCan = this.fuelTank.getFluidAmount() % 1000;
            int ost = this.fuelTank.getFluidAmount() - (countCan * 1000);
            /*
            int k = 0;
            for(ItemStack stacks : stats.rocketStacks)
            {
            	if(stacks == null) {
            		if(k < countCan)
	            	{
	            		stacks = new ItemStack(GSItems.HeliumHydrogenBucket);
	            		k++;
	            	}
	            	
	            	if(k >= countCan && ost > 0)
	            	{
	            		stacks = new ItemStack(GSItems.HeliumHydrogenCanister, 1, ItemCanisterGeneric.EMPTY - ost);
	            		ost = 0;
	            	}
            	}
            }*/
    ...
    public List<ItemStack> getItemsDropped(List<ItemStack> droppedItems)
    {
        ...
        ItemStack rocket = new ItemStack(GSItems.Tier5Rocket, 1, this.rocketType.getIndex());
