...
import galaxyspace.systems.SolarSystem.planets.overworld.tile.TileEntityCargoFluidUnloader;
...
public class BlockCargoFluidLoader extends BlockAdvancedTile implements ItemBlockDesc.IBlockShiftDesc
{
    ...
    public BlockCargoFluidLoader(String assetName)
    {
        super(Material.rock);
    ...
    {
        par3List.add(new ItemStack(par1, 1, BlockCargoFluidLoader.METADATA_CARGO_LOADER));
        par3List.add(new ItemStack(par1, 1, BlockCargoFluidLoader.METADATA_CARGO_UNLOADER));
    }
    ...
    public void onBlockAdded(World world, int x, int y, int z)
    {
        ...
        if (tileEntity != null)
        {
            if (tileEntity instanceof TileEntityCargoFluidLoader)
            {
                ((TileEntityCargoFluidLoader) tileEntity).checkForCargoEntity();
            }
            else if (tileEntity instanceof TileEntityCargoFluidUnloader)
            {
                ((TileEntityCargoFluidUnloader) tileEntity).checkForCargoEntity();
            }
        }
    }
    ...
    public IIcon getIcon(int side, int metadata)
    {
        ...
        if (metadata >= BlockCargoFluidLoader.METADATA_CARGO_UNLOADER)
        {
            shiftedMeta -= BlockCargoFluidLoader.METADATA_CARGO_UNLOADER;
        ...
        else if (metadata >= BlockCargoFluidLoader.METADATA_CARGO_LOADER)
        {
            shiftedMeta -= BlockCargoFluidLoader.METADATA_CARGO_LOADER;
    ...
    public TileEntity createTileEntity(World world, int metadata)
    {
        if (metadata < BlockCargoFluidLoader.METADATA_CARGO_UNLOADER)
        {
            return new TileEntityCargoFluidLoader();
        }
        else
        {
            return new TileEntityCargoFluidUnloader();
        }
    }
    ...
    public boolean onUseWrench(World world, int x, int y, int z, EntityPlayer par5EntityPlayer, int side, float hitX, float hitY, float hitZ)
    {
        ...
        if (metadata >= BlockCargoFluidLoader.METADATA_CARGO_UNLOADER)
        {
            baseMeta = BlockCargoFluidLoader.METADATA_CARGO_UNLOADER;
        }
        else if (metadata >= BlockCargoFluidLoader.METADATA_CARGO_LOADER)
        {
            baseMeta = BlockCargoFluidLoader.METADATA_CARGO_LOADER;
        }
    ...
    public int damageDropped(int metadata)
    {
        if (metadata >= BlockCargoFluidLoader.METADATA_CARGO_UNLOADER)
        {
            return BlockCargoFluidLoader.METADATA_CARGO_UNLOADER;
        }
        else if (metadata >= BlockCargoFluidLoader.METADATA_CARGO_LOADER)
        {
            return BlockCargoFluidLoader.METADATA_CARGO_LOADER;
        }
    ...
    public String getShiftDescription(int meta)
    {
        ...
        case METADATA_CARGO_LOADER:
            return GCCoreUtil.translate("tile.cargoFluidLoader.description");
        case METADATA_CARGO_UNLOADER:
            return GCCoreUtil.translate("tile.cargoFluidUnloader.description");
        }
        return "";
    }
