...
public class TileEntitySolarWind extends TileBaseUniversalElectricalSource implements IMultiBlock, IPacketReceiver, IDisableableMachine, IInventory, ISidedInventory, IConnector
{
    ...
    public static final float MAX_GENERATE_WATTS = 366 * GSConfigEnergy.coefficientSolarWindPanel;
    @NetworkedField(targetSide = Side.CLIENT)
    public float generateWatts = 0;

    private boolean initialised = false;

    public TileEntitySolarWind()
    {
        this(1);
    }
     ...
     */
    public TileEntitySolarWind(int tier)
    {
    	this.storage.setMaxExtract(TileEntitySolarWind.MAX_GENERATE_WATTS);
        this.storage.setMaxReceive(TileEntitySolarWind.MAX_GENERATE_WATTS);
        if (tier == 1)
        {
    ...
    public void updateEntity()
    {
        if (!this.initialised)
        {
            ...
            if (metadata >= BlockSolarWindPanel.BASIC_METADATA)
            {
                this.storage.setCapacity(40000);
            }
            this.initialised = true;
        ...
        if (!this.worldObj.isRemote)
        {
            if (this.getGenerate() > 0.0F)
            {
            		...
            		this.generateWatts = Math.min(Math.max(this.getGenerate(), 0), TileEntitySolarWind.MAX_GENERATE_WATTS);
    ...
    public int getGenerate()
    {
        ...
        return MathHelper.floor_float(0.2F * difference * difference * (this.solarStrength * (Math.abs(difference) * 800.0F)) * this.getSolarBoost());
    }

    public float getSolarBoost()
    {
		if(this.worldObj.provider instanceof WorldProviderSurface) return 0.037F;
        return (float) (this.worldObj.provider instanceof IAdvancedSpace ? ((IAdvancedSpace) this.worldObj.provider).getSolarWindMultiplier() : 1.0F);
    }
	...
	*/
    ...
    {
        return GCCoreUtil.translate(this.tierGC == 1 ? "tile.SolarWindPanel.name" : "tile.AdvSolarWindPanel.name");
    }
