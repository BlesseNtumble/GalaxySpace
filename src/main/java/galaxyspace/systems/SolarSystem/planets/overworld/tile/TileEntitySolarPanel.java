...
public class TileEntitySolarPanel extends TileBaseUniversalElectricalSource implements IMultiBlock, IPacketReceiver, IDisableableMachine, IInventory, ISidedInventory, IConnector
{
    ...
    {
        this.storage.setMaxExtract(TileEntitySolarPanel.MAX_GENERATE_WATTS);
        this.storage.setMaxReceive(TileEntitySolarPanel.MAX_GENERATE_WATTS);
    ...
    public void updateEntity()
    {
       ...
       /* if (this.tierGC == 1)
        ...
        {*/
            if (celestialAngle > 30 && celestialAngle < 150 && !this.worldObj.isRaining())
            {
        ...
        if (!this.worldObj.isRemote)
        {
            ...
            {
                this.generateWatts = Math.min(Math.max(this.getGenerate(), 0), TileEntitySolarPanel.MAX_GENERATE_WATTS);
            }
            else
            {
    ...
    public int getGenerate()
    {
        ...
        return MathHelper.floor_float((0.01F * difference * difference * (this.solarStrength * (Math.abs(difference) * 500.0F)) * this.getSolarBoost()) * (this.containingItems[1] != null ? 4 : 2 * GSConfigEnergy.coefficientSolarPanel));
    }
    ...
    public void onCreate(BlockVec3 placedPosition)
    {
        ...
        final BlockVec3 vecStrut1 = new BlockVec3(placedPosition.x, placedPosition.y + 2, placedPosition.z);
        ((BlockMulti) GCBlocks.fakeBlock).makeFakeBlock(this.worldObj, vecStrut1, placedPosition, 1);
	...
	*/
    ...
    {
        return GCCoreUtil.translate(this.tierGC == 1 ? "tile.SolarPanel.name" : "tile.SolarPanel.name");
    }
