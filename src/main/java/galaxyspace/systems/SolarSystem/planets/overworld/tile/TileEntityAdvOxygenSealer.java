...
public class TileEntityAdvOxygenSealer extends TileEntityOxygenSealer implements IInventory, ISidedInventory {
   @NetworkedField(
   ...
   public static ArrayList<TileEntityAdvOxygenSealer> loadedTiles = new ArrayList();
   private static final int UNSEALED_OXYGENPERTICK = 12;

   public TileEntityAdvOxygenSealer() {
   ...
   {
      super.validate();
      if (!this.worldObj.isRemote && !loadedTiles.contains(this)) {
   ...
   public int getScaledThreadCooldown(int i) {
      return this.active ? Math.min(i, (int)Math.floor((double)(this.stopSealThreadCooldown * i) / (double)this.threadCooldownTotal)) : 0;
   }
   ...
   public int getFindSealChecks() {
      if (this.active && !(this.storedOxygen < this.oxygenPerTick) && this.hasEnoughEnergyToRun) {
         ...
         return !(blockAbove instanceof BlockAir) && !OxygenPressureProtocol.canBlockPassAir(this.worldObj, blockAbove, new BlockVec3(this.xCoord, this.yCoord + 1, this.zCoord), 1) ? 0 : 1250;
      } else {
         return 0;
      }
   }
   ...
   public void updateEntity()
   {
      if (!this.worldObj.isRemote) {
         ...
         if (oxygenItemStack != null && oxygenItemStack.getItem() instanceof IItemOxygenSupply) {
            ...
            float oxygenDraw = Math.min(30.0F, this.maxOxygen - this.storedOxygen);
   ...
   public static HashMap<BlockVec3, TileEntityAdvOxygenSealer> getSealersAroundAdv(World world, int x, int y, int z, int rSquared) {
      HashMap<BlockVec3, TileEntityAdvOxygenSealer> ret = new HashMap();
      Iterator var6 = (new ArrayList(loadedTiles)).iterator();

      while(var6.hasNext()) {
         TileEntityAdvOxygenSealer tile = (TileEntityAdvOxygenSealer)var6.next();
         if (tile != null && tile.getWorldObj() == world && tile.getDistanceFrom((double)x, (double)y, (double)z) < (double)rSquared) {
   ...
   {
      TileEntityAdvOxygenSealer ret = null;
      double dist = 96 * 96D;

      for (Object tile : world.loadedTileEntityList)
      {
         if (tile instanceof TileEntityAdvOxygenSealer)
         {
            double testDist = ((TileEntityAdvOxygenSealer) tile).getDistanceFrom(x, y, z);
            if (testDist < dist)
            {
               dist = testDist;
               ret = (TileEntityAdvOxygenSealer) tile;
            }
         }
      }
