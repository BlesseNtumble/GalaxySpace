...
public class ContainerAdvCircuitFabricator extends Container
{
    private TileEntityAdvCircuitFabricator tileEntity;

    public ContainerAdvCircuitFabricator(InventoryPlayer playerInv, TileEntityAdvCircuitFabricator tileEntity)
    {
        ...
        for(slot = 0; slot < 4; ++slot)        
        	this.addSlotToContainer(new SlotUpgrades(tileEntity, 7 + slot, 177, 18 + (21 * slot), 1, ItemUpgrades.class));
       
        for (slot = 0; slot < 3; ++slot)
        {
            for (int var4 = 0; var4 < 9; ++var4)
            {
                this.addSlotToContainer(new Slot(playerInv, var4 + slot * 9 + 9, 8 + var4 * 18, 127 + slot * 18));
            }
        }

        for (slot = 0; slot < 9; ++slot)
        {
           this.addSlotToContainer(new Slot(playerInv, slot, 8 + slot * 18, 185));
        }
    }
