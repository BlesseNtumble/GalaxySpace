...
extends GuiContainerGC {
    ...
    private final TileEntitySolarWind solarwindPanel;
    private GuiButton buttonEnableSolar;
    private GuiElementInfoRegion electricInfoRegion;

    public GuiSolarWind(InventoryPlayer par1InventoryPlayer, TileEntitySolarWind solarwindPanel) {
        super((Container)new ContainerSolarWind(par1InventoryPlayer, solarwindPanel));
        ...
        this.solarwindPanel = solarwindPanel;
        this.ySize = 201;
        this.xSize = 176;
    }
    ...
    protected void actionPerformed(GuiButton par1GuiButton) {
        switch (par1GuiButton.id) {
            case 0: {
                GalacticraftCore.packetPipeline.sendToServer((IPacket)new PacketSimple(PacketSimple.EnumSimplePacket.S_UPDATE_DISABLEABLE_BUTTON, new Object[]{this.solarwindPanel.xCoord, this.solarwindPanel.yCoord, this.solarwindPanel.zCoord, 0}));
            }
        }
    }
    ...
    public void initGui() {
        ...
        electricityDesc.add(EnumColor.YELLOW + GCCoreUtil.translate((String)"gui.energyStorage.desc.1") + (int)Math.floor(this.solarwindPanel.getEnergyStoredGC()) + " / " + (int)Math.floor(this.solarwindPanel.getMaxEnergyStoredGC()));
        ...
        float sunVisible = (float)Math.round((float)this.solarwindPanel.solarStrength / 9.0f * 1000.0f) / 10.0f;
        sunGenDesc.add(this.solarwindPanel.solarStrength > 0 ? GCCoreUtil.translate((String)"gui.status.sunVisible.name") + ": " + sunVisible + "%" : GCCoreUtil.translate((String)"gui.status.blockedfully.name"));
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        int offsetY = 35;
        this.buttonEnableSolar.enabled = this.solarwindPanel.disableCooldown == 0;
        this.buttonEnableSolar.displayString = !this.solarwindPanel.getDisabled(0) ? GCCoreUtil.translate((String)"gui.button.disable.name") : GCCoreUtil.translate((String)"gui.button.enable.name");
        String displayString = this.solarwindPanel.getInventoryName();
        this.fontRendererObj.drawString(displayString, this.xSize / 2 - this.fontRendererObj.getStringWidth(displayString) / 2, 7, 0x404040);
        displayString = GCCoreUtil.translate((String)"gui.message.status.name") + ": " + this.getStatus();
        this.fontRendererObj.drawString(displayString, this.xSize / 2 - this.fontRendererObj.getStringWidth(displayString) / 2, 22 + offsetY, 0x404040);
        displayString = GCCoreUtil.translate((String)"gui.message.generating.name") + ": " + (this.solarwindPanel.generateWatts > 0.0f ? EnergyDisplayHelper.getEnergyDisplayS((float)this.solarwindPanel.generateWatts) + "/t" : GCCoreUtil.translate((String)"gui.status.notGenerating.name"));
        this.fontRendererObj.drawString(displayString, this.xSize / 2 - this.fontRendererObj.getStringWidth(displayString) / 2, 11 + offsetY, 0x404040);
        float boost = (float)Math.round((this.solarwindPanel.getSolarBoost() - 1.0f) * 1000.0f) / 10.0f;
        displayString = GCCoreUtil.translate((String)"gui.message.environment.name") + ": " + boost + "%";
        this.fontRendererObj.drawString(displayString, this.xSize / 2 - this.fontRendererObj.getStringWidth(displayString) / 2, 33 + offsetY, 0x404040);
        this.fontRendererObj.drawString(GCCoreUtil.translate((String)"container.inventory"), 8, this.ySize - 94, 0x404040);
    }

    private String getStatus() {
        if (this.solarwindPanel.getDisabled(0)) {
            return EnumColor.ORANGE + GCCoreUtil.translate((String)"gui.status.disabled.name");
        }
        if (!this.solarwindPanel.getWorldObj().isDaytime()) {
            return EnumColor.DARK_RED + GCCoreUtil.translate((String)"gui.status.blockedfully.name");
        }
        if (this.solarwindPanel.getWorldObj().isRaining() || this.solarwindPanel.getWorldObj().isThundering()) {
            return EnumColor.DARK_RED + GCCoreUtil.translate((String)"gui.status.raining.name");
        }
        if (this.solarwindPanel.solarStrength == 0) {
            return EnumColor.DARK_RED + GCCoreUtil.translate((String)"gui.status.blockedfully.name");
        }
        if (this.solarwindPanel.solarStrength < 9) {
            return EnumColor.DARK_RED + GCCoreUtil.translate((String)"gui.status.blockedpartial.name");
        }
        if (this.solarwindPanel.generateWatts > 0.0f) {
    ...
    protected void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
        ...
        EnergyDisplayHelper.getEnergyDisplayTooltip((float)this.solarwindPanel.getEnergyStoredGC(), (float)this.solarwindPanel.getMaxEnergyStoredGC(), electricityDesc);
        this.electricInfoRegion.tooltipStrings = electricityDesc;
        if (this.solarwindPanel.getEnergyStoredGC() > 0.0f) {
            this.drawTexturedModalRect(var5 + 83, var6 + 24, 176, 0, 11, 10);
        }
        if (this.solarwindPanel.solarStrength > 0) {
            this.drawTexturedModalRect(var5 + 48, var6 + 21, 176, 10, 16, 16);
        }
        this.drawTexturedModalRect(var5 + 97, var6 + 25, 187, 0, Math.min(this.solarwindPanel.getScaledElecticalLevel(54), 54), 7);
    }
}
