...
public class TileEntityWindTurbine extends TileBaseUniversalElectricalSource implements IMultiBlock, IPacketReceiver, IDisableableMachine, IInventory, ISidedInventory, IConnector
{
     ...
     */
    public TileEntityWindTurbine(int tier)
    {
        this.storage.setMaxExtract(TileEntityWindTurbine.MAX_GENERATE_WATTS);
        this.storage.setMaxReceive(TileEntityWindTurbine.MAX_GENERATE_WATTS);
        if (tier == 2)
        {
    ...
    public void updateEntity()
    {
        ...
        if (!this.worldObj.isRemote)
        {
            if (this.getGenerate() > 0.0F && this.worldObj.canBlockSeeTheSky(this.xCoord, this.yCoord, this.zCoord))
            {
                this.generateWatts = Math.min(Math.max(this.getGenerate(), 0), TileEntityWindTurbine.MAX_GENERATE_WATTS);
            }
            else
            {
    ...
    public float getGenerate()
    {
       if (this.getDisabled(0))
        ...
        return MathHelper.floor_float((50.0F + 5.0F * Math.round(this.yCoord / 10.0F)) * this.getWindBoost()) * (GSConfigEnergy.coefficientWindTurbine);
       
    }

    public float getWindBoost()
    {
    	if(this.worldObj.provider instanceof IGalacticraftWorldProvider && !(this.worldObj.provider instanceof WorldProviderSpaceStation))
    		if(this.worldObj.canBlockSeeTheSky(this.xCoord, this.yCoord, this.zCoord))
    			return ((IGalacticraftWorldProvider) this.worldObj.provider).getWindLevel();
    ...
    public void onCreate(BlockVec3 placedPosition)
    {
    	...
    	for(int i = 1; i <= 2; i++)
    	{
    		final BlockVec3 vecStrut = new BlockVec3(placedPosition.x, placedPosition.y + i, placedPosition.z);
    		((BlockMulti) GCBlocks.fakeBlock).makeFakeBlock(this.worldObj, vecStrut, placedPosition, 1);
    	}
        ...
        for (int x = 0; x < 1; x++)
        {
            for (int z = 0; z < 1; z++)
            {
                final BlockVec3 vecToAdd = new BlockVec3(placedPosition.x + x, placedPosition.y + 3, placedPosition.z + z);
        ...
        for (int y = 1; y <= 3; y++)
        {
            for (int x = 0; x < 1; x++)
            {
                for (int z = 0; z < 1; z++)
                {
                    ...
                    {
                        FMLClientHandler.instance().getClient().effectRenderer.addBlockDestroyEffects(thisBlock.x + (y == 3 ? x : 0), thisBlock.y + y, thisBlock.z + (y == 3 ? z : 0), GSBlocks.SolarPanel, Block.getIdFromBlock(GSBlocks.SolarPanel) >> 12 & 255);
                    }

                    this.worldObj.setBlockToAir(thisBlock.x + (y == 3 ? x : 0), thisBlock.y + y, thisBlock.z + (y == 3 ? z : 0));
                }
            }
        }
	...
	*/
    ...
    {
        return GCCoreUtil.translate(this.tierGC == 1 ? "tile.WindTurbine.name" : "tile.WindTurbine.name");
    }
