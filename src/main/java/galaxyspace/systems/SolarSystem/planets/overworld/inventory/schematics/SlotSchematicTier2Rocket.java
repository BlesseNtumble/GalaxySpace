...
public class SlotSchematicTier2Rocket extends Slot
{
    ...
    public boolean isItemValid(ItemStack par1ItemStack)
    {
    	...
    	List<INasaWorkbenchRecipe> recipes = GSRecipeUtil.getRocketT2Recipes();
    	for (INasaWorkbenchRecipe recipe : recipes)
        {
    		if(recipe.getRecipeInput().get(this.index) == null) return true;
    		
            if (par1ItemStack.isItemEqual(recipe.getRecipeInput().get(this.index)))
                return true;
        }
        return false;
