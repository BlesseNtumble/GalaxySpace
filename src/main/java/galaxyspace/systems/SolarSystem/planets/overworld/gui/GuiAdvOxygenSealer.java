...
public class GuiAdvOxygenSealer extends GuiContainerGC
{
    ...
    public GuiAdvOxygenSealer(InventoryPlayer par1InventoryPlayer, TileEntityAdvOxygenSealer par2TileEntityAirDistributor)
    {
        super(new ContainerAdvOxygenSealer(par1InventoryPlayer, par2TileEntityAirDistributor));
    ...
    {
        this.fontRendererObj.drawString(EnumColor.WHITE + this.sealer.getInventoryName(), 38 + this.sealer.getInventoryName().length(), 6, 4210752);
...
//        }
        ...
        GalaxySpace.debug("" + threadCooldown);
...
//			electricityDesc.add(EnumColor.YELLOW + GCCoreUtil.translate("gui.energyStorage.desc.1") + ": " + ((int) Math.floor(this.sealer.getEnergyStoredGC()) + " / " + (int) Math.floor(this.sealer.getMaxEnergyStoredGC())));
            ...
            this.drawTexturedModalRect(var5 + 148, var6 + 60, 230, 142, 10, 27);
            if (scale != 0)
            {
                this.drawTexturedModalRect(var5 + 149, var6 + 61 + scale, 240, 142, 8, 25 - scale);
            }

            this.drawTexturedModalRect(var5 + 16, var6 + 107, 192, 47, 56, 9);
            this.drawTexturedModalRect(var5 + 4, var6 + 107, 192, 56, 11, 10);
            
            if (this.sealer.getEnergyStoredGC() > 0)
            {
            	scale = this.sealer.getScaledElecticalLevel(55);
                this.drawTexturedModalRect(var5 + 116 - 99, var6 + 108, 192, 0, scale, 7);
                this.drawTexturedModalRect(var5 + 3, var6 + 107, 192, 7, 11, 10);   
    	       ...
    	       /* if(!(this.inventorySlots.getSlot(i).inventory instanceof InventoryPlayer))
    	        {*/
    		        ...
    		        switch(i)
    		        {
    		        	...
    		        	case 2:
    		        	{
    		        		this.drawTexturedModalRect(var5 + x - 2, var6 + y - 2, 192, 175, 20, 21);	        		 
    		        		break;
    		        	}
    		        	default: 
    		        	{
    		        		this.drawTexturedModalRect(var5 + x - 2, var6 + y - 2, 192, 26, 20, 21);
    		        		break;
    		        	}	        	
    		        }
    		        GL11.glPopMatrix();
