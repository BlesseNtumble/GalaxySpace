...
public class AssemberRecipes
{
	public static final AssemberRecipes instance = new AssemberRecipes();
    private List<IRecipe> recipes = new ArrayList<IRecipe>();

    public ShapedRecipes addRecipe(ItemStack output, Object... inputList)
    {
        String s = "";
        int i = 0;
        int j = 0;
    ...
    public void addShapelessRecipe(ItemStack par1ItemStack, Object... par2ArrayOfObj)
    {
        ...
        recipes.add(new ShapelessOreRecipe(par1ItemStack, arraylist.toArray()));
    }

    public ItemStack findMatchingRecipe(IInventory inventory, World par2World)
    {
        int i = 0;
        ...
        else
        {
            for (j = 0; j < recipes.size(); ++j)
            {
                ...
                if (irecipe instanceof ShapedRecipes && AssemberRecipes.matches((ShapedRecipes) irecipe, inventory, par2World))
                {
                    return irecipe.getRecipeOutput().copy();
                }
                else if (irecipe instanceof ShapelessOreRecipe && AssemberRecipes.matchesShapeless((ShapelessOreRecipe) irecipe, inventory, par2World))
                {
                    return irecipe.getRecipeOutput().copy();
                }
            }
    ...
    private static boolean matches(ShapedRecipes recipe, IInventory inventory, World par2World)
    {
        for (int i = 0; i <= 3 - recipe.recipeWidth; ++i)
        {
            ...
            {
                if (AssemberRecipes.checkMatch(recipe, inventory, i, j, true))
                {
                    return true;
                }

                if (AssemberRecipes.checkMatch(recipe, inventory, i, j, false))
                {
                    return true;
                }
            }
