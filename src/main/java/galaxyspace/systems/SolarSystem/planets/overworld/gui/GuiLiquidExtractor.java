...
public class GuiLiquidExtractor extends GuiContainerGC
{
    ...
    public void initGui()
    {
       ...
       /*
        ...
        batterySlotDesc.add(GCCoreUtil.translate("gui.module_electrolyse.desc"));
        this.infoRegions.add(new GuiElementInfoRegion((this.width - this.xSize) / 2 + 10, (this.height - this.ySize) / 2 + 40, 18, 18, batterySlotDesc, this.width, this.height, this));
       */
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2)
    {
        ...
        this.fontRendererObj.drawString(str, 128 - this.fontRendererObj.getStringWidth(str) / 2, 104, 4210752);
...
/*
        ...
        processDesc.clear();
        processDesc.add(GCCoreUtil.translate("gui.electricCompressor.desc.0") + ": " + (100 - scale) + "%");
        ...
        //this.drawTexturedModalRect(containerWidth + 104, containerHeight + 44, 192 + 20, 108, 16, 17);
       /* for(int i = 0; i < 8 ; i++) 
        {
        	this.drawTexturedModalRect(containerWidth + 40 + (i * 4), containerHeight + 42, 203, 114, 4, 4);        	
        	this.drawTexturedModalRect(containerWidth + 72 + (i * 4), containerHeight + 66, 203, 114, 4, 4);
        
        	this.drawTexturedModalRect(containerWidth + 72, containerHeight + 42 + (i * 3), 203, 114, 3, 4);
        	this.drawTexturedModalRect(containerWidth + 101, containerHeight + 50 + (i * 2), 203, 114, 3, 4);
        } */ 
       /* if (this.tileEntity.processTicks > 0)
        {
            scale = (int) ((double) this.tileEntity.processTicks / (double) this.tileEntity.processTimeRequired * 16);
            this.drawTexturedModalRect(containerWidth + 104, containerHeight + 44, 192 + 20, 124, 16 - scale, 17);
       
            
            for(int i = 0; i < 8 ; i++) 
            {
            	this.drawTexturedModalRect(containerWidth + 40 + (i * 4), containerHeight + 42, 203, 130, 4, 5);        	
            	this.drawTexturedModalRect(containerWidth + 72 + (i * 4), containerHeight + 66, 203, 130, 4, 5);
            
            	this.drawTexturedModalRect(containerWidth + 72, containerHeight + 42 + (i * 3), 203, 130, 3, 4);
            	this.drawTexturedModalRect(containerWidth + 101, containerHeight + 50 + (i * 2), 203, 130, 3, 4);
            }  
           
            /*if (this.tileEntity.processTicks < this.tileEntity.processTimeRequired / 2)
            {
                this.drawTexturedModalRect(containerWidth + 75, containerHeight + 39, 230, 86, 24, 26);
            //}
        }
        ...
        */
        ...
        {
	        GL11.glPushMatrix();	        
	        GSUtils.drawFluid(fluid, this.width / 2 - 8, this.height / 2 - 62, 16, 38, this.tileEntity.waterTank.getCapacity());
			GL11.glPopMatrix();
        }
