...
public class GSEventHandler {
    ...
    public void onSetBlock(SetBlockEvent e) {
        ...
        if (e.world != null && !e.world.isRemote && e.world.provider instanceof IGalacticraftWorldProvider && e.pos != null && e.block != null) {
            ...
            for (BlockToChange block : block_to_change) {
                ...
                if (e.world.provider instanceof IProviderFreeze && !((IProviderFreeze) e.world.provider).isFreeze())
                    continue;
    ...
    @SubscribeEvent
    public void onTickBlock(LivingUpdateEvent e) {
        ...
        if (e.getEntityLiving() instanceof EntityPlayerMP) {
            ...
            if (!world.isRemote && world.provider instanceof IGalacticraftWorldProvider) {
                ...
                for (Iterator<Chunk> it = worldServer.getPersistentChunkIterable(worldServer.getPlayerChunkMap().getChunkIterator()); it.hasNext(); ) {
                    Chunk chunk = it.next();
                    ...
                    BlockPos pos = chunk.getPrecipitationHeight(new BlockPos(x + world.rand.nextInt(16), 0, z + world.rand.nextInt(16)));
                    ...
                    for (int y = 0; y < pos.getY(); y++) {
                        ...
                        if (block instanceof BlockLeaves) {
                            ...
                            if (!OxygenUtil.isAABBInBreathableAirBlock(world, bb, thermal_check)) {
                                world.setBlockState(pos, GSBlocks.DRY_LEAVES.getDefaultState(), 3);
                            }
                        }
    ...
    public void onUpdateBlocks(UpdateBlockEvent e) {
        ...
        if (!e.world.isRemote) {
            ...
            if (e.world.provider instanceof IGalacticraftWorldProvider) {
                ...
                if (((IGalacticraftWorldProvider) e.world.provider).getCelestialBody().atmosphere.isGasPresent(EnumAtmosphericGas.OXYGEN))
                    return;
...
*/
    ...
    public void onEntityInteract(PlayerInteractEvent.EntityInteract event) {
        ...
        if (!event.getWorld().isRemote) {
            ...
            if (GalaxySpace.debug)
                GalaxySpace.instance.debug(EntityRegistry.getEntry(event.getTarget().getClass()));
    ...
    */
    ...
    @SubscribeEvent
    public void onSuffocationEvent(GCCoreOxygenSuffocationEvent.Pre e) {
        EntityLivingBase living = e.getEntityLiving();

        Set<EntityEntry> list = new HashSet<>();
        for (String string : GSConfigCore.unsuffocation_entity) {
            String[] meta = string.split(":");
            EntityEntry entity = ForgeRegistries.ENTITIES.getValue(new ResourceLocation(meta[0], meta[1]));
            if (entity != null)
                list.add(entity);
        }

        e.setCanceled(list.contains(EntityRegistry.getEntry(living.getClass())));
    }
