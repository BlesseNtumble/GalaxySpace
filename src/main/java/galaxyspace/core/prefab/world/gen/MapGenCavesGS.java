...
public class MapGenCavesGS extends MapGenBaseMeta {
	public static final int BREAK_THROUGH_CHANCE = 25; // 1 in n chance

	
	public IBlockState surface_block;
	public IBlockState subsurface_block;
	public IBlockState stone_block;
	
	public IBlockState air_block;
	public int y_down;

	public MapGenCavesGS(IBlockState surface, IBlockState subsurface, IBlockState stone, IBlockState airindown, int y) {
		this.surface_block = surface;
		this.subsurface_block = subsurface;
		this.stone_block = stone;
		this.air_block = airindown;
		this.y_down = y;
	}
	...
	protected void generateCaveNode(long par1, int par3, int par4, ChunkPrimer primer, double par6, double par8, double par10, float par12, float par13, float par14, int par15, int par16, double par17) {
		...
		for (final boolean flag1 = random.nextInt(6) == 0; par15 < par16; ++par15) {
			final double d6 = 1.5D + MathHelper.sin(par15 * 3.14F / par16) * par12 * 1.0F;
			...
			if (!flag && par15 == k1 && par12 > 1.0F && par16 > 0) {
				this.generateCaveNode(random.nextLong(), par3, par4, primer, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 - 3.14F / 2F, par14 / 3.0F, par15, par16, 1.0D);
				this.generateCaveNode(random.nextLong(), par3, par4, primer, par6, par8, par10, random.nextFloat() * 0.5F + 0.5F, par13 + 3.14F / 2F, par14 / 3.0F, par15, par16, 1.0D);
				return;
			}
								...
								for (int localZ = l2; localZ < i3; localZ++) {
									...
									if (xfactorSq + zfactorSq < 1.0D) {
										if (yfactor > -0.7D && xfactorSq + yfactorSq + zfactorSq < 1.0D) {
											...
											if(state.equals(this.surface_block) && random.nextInt(BREAK_THROUGH_CHANCE) == 0)
											{
												primer.setBlockState(localX, localY, localZ, Blocks.AIR.getDefaultState());
											}
											else if(state.equals(this.subsurface_block) || state.equals(this.stone_block))
											{
												if(localY <= this.y_down) 
													primer.setBlockState(localX, localY, localZ, this.air_block);
												else
													primer.setBlockState(localX, localY, localZ, Blocks.AIR.getDefaultState());
											}
											/*
											if (block == this.surface_block.getBlock()) {
												if (metadata == this.subsurface_block.getBlock().getMetaFromState(state) || metadata == this.stoneMeta) {
													primer.setBlockState(localX, localY, localZ, Blocks.AIR.getDefaultState());
												} else if (metadata == this.surfaceMeta && random.nextInt(BREAK_THROUGH_CHANCE) == 0) {
													primer.setBlockState(localX, localY, localZ, Blocks.AIR.getDefaultState());
												}
											}
											*/
											/*
											if(block == GSBlocks.SURFACE_ICE)
											{
												primer.setBlockState(localX, localY, localZ, Blocks.AIR.getDefaultState());
											}*/
										}
	...
	protected void recursiveGenerate(World par1World, int par2, int par3, int par4, int par5, ChunkPrimer primer) {
		...
		for (int var8 = 0; var8 < var7; ++var8) {
			...
			for (int var16 = 0; var16 < var15; ++var16) {
				final float var17 = this.rand.nextFloat() * 3.14F * 2F;
