...
public class ItemFluidCargoRocket  extends Item implements IHoldableItem, ISortableItem, IModificationItem
{
    public ItemFluidCargoRocket(String assetName)
    {
        super();
        this.setMaxDamage(0);
    ...
    public void addInformation(ItemStack par1ItemStack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn)
    {
        ...
        if (par1ItemStack.hasTagCompound())
        {
        	if(par1ItemStack.getTagCompound().hasKey("RocketFuel")) {
        		EntityCustomCargoRocket rocket = new EntityCustomCargoRocket(FMLClientHandler.instance().getWorldClient(), 0, 0, 0, EnumRocketType.values()[par1ItemStack.getItemDamage()]);
            ...
            for(Engine_Type engines : Engine_Type.values()) {
	        	if(par1ItemStack.getTagCompound().hasKey(engines.getName()))
	        	{
	        		...
	        		EntityCustomCargoRocket rocket = new EntityCustomCargoRocket(FMLClientHandler.instance().getWorldClient(), 0, 0, 0, EnumRocketType.values()[meta], engines);
    ...
    public static boolean placeRocketOnPad(ItemStack stack, World worldIn, TileEntity tile, float centerX, float centerY, float centerZ)
    {
        ...
        EntityCustomCargoRocket rocket = new EntityCustomCargoRocket(worldIn, centerX, centerY, centerZ, EnumRocketType.values()[stack.getItemDamage()]);
/*
        for(Engine_Type engines : Engine_Type.values())
