...
public class CustomCloudRender extends IRenderHandler implements ISelectiveResourceReloadListener
{
    // Shared constants.
    ...
    public CustomCloudRender(float[] layersLevels_t)
    {
        ...
        layersLevels = layersLevels_t;
        offsetX = new float[layersLevels_t.length];
        offsetZ = new float[layersLevels_t.length];
        for(int i = 0; i < layersLevels_t.length; i++) {
    ...
    public boolean render(int cloudTicks, float partialTicks, float[] offsets)
    {
        if (!isBuilt())
            return false;
        ...
        GlStateManager.translate((offU * scale) - x, y + offsets[1], (offV * scale) - z);
