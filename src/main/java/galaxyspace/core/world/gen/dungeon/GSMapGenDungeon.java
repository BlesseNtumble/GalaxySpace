...
public class GSMapGenDungeon
{
    public ArrayList<GSDungeonRoom> bossRooms = new ArrayList<GSDungeonRoom>();
    public ArrayList<GSDungeonRoom> treasureRooms = new ArrayList<GSDungeonRoom>();
    public ArrayList<GSDungeonRoom> otherRooms = new ArrayList<GSDungeonRoom>();

    public final Block DUNGEON_WALL_ID;
    public final Block DUNGEON_TOP_ID;
    public final Block DUNGEON_FLOOR_ID;
    public final int DUNGEON_WALL_META;
    public final int RANGE;
    public final int HALLWAY_LENGTH;
    public final int HALLWAY_HEIGHT;
    public final int DUNGEON_TOP_META;
    public final int DUNGEON_FLOOR_META;
    
    public static boolean useArrays = false;

    public World worldObj;

    private final ArrayList<GSDungeonRoom> rooms = new ArrayList<GSDungeonRoom>();

    public GSMapGenDungeon(Block wallID, int wallMeta, int range, int hallwayLength, int hallwayHeight, int topmeta, int floormeta)
    {
         ...
         this.DUNGEON_TOP_ID = wallID;
         this.DUNGEON_FLOOR_ID = wallID;
         
         this.DUNGEON_TOP_META = topmeta;
         this.DUNGEON_FLOOR_META = floormeta;
    }
    public GSMapGenDungeon(Block wallID, int wallMeta, int range, int hallwayLength, int hallwayHeight, Block topID, int topmeta, Block floorID, int floormeta)
    {
        this.DUNGEON_WALL_ID = wallID;
        this.DUNGEON_WALL_META = wallMeta;
        this.RANGE = range;
        ...
        this.DUNGEON_TOP_ID = topID;
        this.DUNGEON_FLOOR_ID = floorID;
        this.DUNGEON_TOP_META = topmeta;
        this.DUNGEON_FLOOR_META = floormeta;
    }
    ...
    {
        GSMapGenDungeon.useArrays = useArrays;
        this.worldObj = world;

        final List<GSDungeonBoundingBox> boundingBoxes = new ArrayList<GSDungeonBoundingBox>();

        final int length = rand.nextInt(4) + 5;

        GSDungeonRoom currentRoom = GSDungeonRoom.makeRoom(this, rand, x, y, z, ForgeDirection.DOWN);
        currentRoom.generate(blocks, metas, chunkX, chunkZ);
        this.rooms.add(currentRoom);
        final GSDungeonBoundingBox cbb = currentRoom.getBoundingBox();
        ...
        for (int i = 0; i <= length; i++)
        {
            for (int j = 0; j < 8; j++)
            {
                ...
                GSDungeonRoom possibleRoom = GSDungeonRoom.makeRoom(this, rand, currentRoom.posX + offsetX, y, currentRoom.posZ + offsetZ, entranceDir.getOpposite());// this.getOppositeDir(entranceDir));
                if (i == length - 1)
                {
                    possibleRoom = GSDungeonRoom.makeBossRoom(this, rand, currentRoom.posX + offsetX, y, currentRoom.posZ + offsetZ, entranceDir.getOpposite()); // this.getOppositeDir(entranceDir));
                }
                if (i == length)
                {
                    possibleRoom = GSDungeonRoom.makeTreasureRoom(this, rand, currentRoom.posX + offsetX, y, currentRoom.posZ + offsetZ, entranceDir.getOpposite()); // this.getOppositeDir(entranceDir));
                }
                final GSDungeonBoundingBox possibleRoomBb = possibleRoom.getBoundingBox();
                final GSDungeonBoundingBox currentRoomBb = currentRoom.getBoundingBox();
                if (!this.isIntersecting(possibleRoomBb, boundingBoxes))
                {
                    ...
                    {
                        GSDungeonBoundingBox corridor1 = null;
                        switch (dir)
                        // East = 0, North = 1, South = 2, West = 3
                        {
                        case EAST: // East z++
                            corridor1 = new GSDungeonBoundingBox(corridorX - 1, currentRoomBb.maxZ, corridorX, possibleRoomBb.minZ - 1);
                            break;
                        case NORTH: // North x++
                            corridor1 = new GSDungeonBoundingBox(currentRoomBb.maxX, corridorZ - 1, possibleRoomBb.minX - 1, corridorZ);
                            break;
                        case SOUTH: // South x--
                            corridor1 = new GSDungeonBoundingBox(possibleRoomBb.maxX, corridorZ - 1, currentRoomBb.minX - 1, corridorZ);
                            break;
                        case WEST: // West z--
                            corridor1 = new GSDungeonBoundingBox(corridorX - 1, possibleRoomBb.maxZ, corridorX, currentRoomBb.minZ - 1);
                            break;
                        default:
                            break;
                        }
                    ...
                    {
                        GSDungeonBoundingBox corridor1 = null;
                        GSDungeonBoundingBox corridor2 = null;
                        ...
                        {
                        case EAST: // East z++
                            corridor1 = new GSDungeonBoundingBox(curCenterX - 1, currentRoomBb.maxZ, curCenterX + 1, possibleCenterZ - 1);
                            if (offsetX > 0) // x++
                            {
                                corridor2 = new GSDungeonBoundingBox(corridor1.minX - extraLength, corridor1.maxZ + 1, possibleRoomBb.minX, corridor1.maxZ + 3);
                                dir2 = ForgeDirection.NORTH;
                            }
                            else
                            // x--
                            {
                                corridor2 = new GSDungeonBoundingBox(possibleRoomBb.maxX, corridor1.maxZ + 1, corridor1.maxX + extraLength, corridor1.maxZ + 3);
                                dir2 = ForgeDirection.SOUTH;
                            }
                            break;
                        case NORTH: // North x++
                            corridor1 = new GSDungeonBoundingBox(currentRoomBb.maxX, curCenterZ - 1, possibleCenterX - 1, curCenterZ + 1);
                            if (offsetZ > 0) // z++
                            {
                                corridor2 = new GSDungeonBoundingBox(corridor1.maxX + 1, corridor1.minZ - extraLength, corridor1.maxX + 4, possibleRoomBb.minZ);
                                dir2 = ForgeDirection.EAST;
                            }
                            else
                            // z--
                            {
                                corridor2 = new GSDungeonBoundingBox(corridor1.maxX + 1, possibleRoomBb.maxZ, corridor1.maxX + 4, corridor1.maxZ + extraLength);
                                dir2 = ForgeDirection.WEST;
                            }
                            break;
                        case SOUTH: // South x--
                            corridor1 = new GSDungeonBoundingBox(possibleCenterX + 1, curCenterZ - 1, currentRoomBb.minX - 1, curCenterZ + 1);
                            if (offsetZ > 0) // z++
                            {
                                corridor2 = new GSDungeonBoundingBox(corridor1.minX - 3, corridor1.minZ - extraLength, corridor1.minX - 1, possibleRoomBb.minZ);
                                dir2 = ForgeDirection.EAST;
                            }
                            else
                            // z--
                            {
                                corridor2 = new GSDungeonBoundingBox(corridor1.minX - 3, possibleRoomBb.maxZ, corridor1.minX - 1, corridor1.maxZ + extraLength);
                                dir2 = ForgeDirection.WEST;
                            }
                            break;
                        case WEST: // West z--
                            corridor1 = new GSDungeonBoundingBox(curCenterX - 1, possibleCenterZ + 1, curCenterX + 1, currentRoomBb.minZ - 1);
                            if (offsetX > 0) // x++
                            {
                                corridor2 = new GSDungeonBoundingBox(corridor1.minX - extraLength, corridor1.minZ - 3, possibleRoomBb.minX, corridor1.minZ - 1);
                                dir2 = ForgeDirection.NORTH;
                            }
                            else
                            // x--
                            {
                                corridor2 = new GSDungeonBoundingBox(possibleRoomBb.maxX, corridor1.minZ - 3, corridor1.maxX + extraLength, corridor1.minZ - 1);
                                dir2 = ForgeDirection.SOUTH;
                            }
                            break;
                        default:
    ...
    private void genCorridor(GSDungeonBoundingBox corridor, Random rand, int y, int cx, int cz, ForgeDirection dir, Block[] blocks, byte[] metas, boolean doubleCorridor)
    {
            ...
            for (int k = corridor.minZ - 1; k <= corridor.maxZ + 1; k++)
            {
                ...
                for (int j = y - 1; j <= y + this.HALLWAY_HEIGHT; j++)
                {
                    ...
                    else
                    {
                    	 if (j == y - 1)
                         {
                    		 this.placeBlock(blocks, metas, i, j, k, cx, cz, this.DUNGEON_FLOOR_ID, this.DUNGEON_FLOOR_META);
                         }
                    	 else this.placeBlock(blocks, metas, i, j, k, cx, cz, this.DUNGEON_WALL_ID, this.DUNGEON_WALL_META);
    ...
    {
        final ArrayList<GSDungeonRoom> rooms = new ArrayList<GSDungeonRoom>();
        rooms.addAll(this.rooms);
        this.rooms.clear();
        for (final GSDungeonRoom room : rooms)
        {
    ...
    protected boolean canGenDungeonAtCoords(long worldSeed, int i, int j)
    {
        ...
        final long dungeonSeed = randX * 341873128712L + randZ * 132897987541L + worldSeed + 4291726 + this.worldObj.provider.dimensionId;
    ...
    public void generateEntranceCrater(Block[] blocks, byte[] meta, int x, int y, int z, int cx, int cz)
    {
        ...
        for (int i = x - range; i < x + range; i++)
        {
            for (int k = z - range; k < z + range; k++)
            {
                ...
                {
                    if ((Blocks.air != this.getBlock(blocks, i, j - 1, k, cx, cz) || this.getBlock(blocks, i, j, k, cx, cz) == this.DUNGEON_TOP_ID) && helper <= depth)
                    {
                        this.placeBlock(blocks, meta, i, j, k, cx, cz, Blocks.air, 0);
                        //this.placeBlock(blocks, meta, x, y, z, cx, cz, Blocks.cobblestone, 0);
                        helper++;
                    }
    ...
    private boolean isIntersecting(GSDungeonBoundingBox bb, List<GSDungeonBoundingBox> dungeonBbs)
    {
        for (final GSDungeonBoundingBox bb2 : dungeonBbs)
        {
