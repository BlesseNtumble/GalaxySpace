...
public abstract class ModelOBJArmor extends ModelBiped {
	...
	public void render(Entity entity, float x, float y, float z, float yaw, float pitch, float parTicks) {
		...
		if (entity instanceof EntityPlayer) {
				...
				if (gearData != null) {
					...
					if (!ClientProxyCore.gearDataRequests.contains(id)) {
						GalacticraftCore.packetPipeline.sendToServer(new PacketSimple(
								PacketSimple.EnumSimplePacket.S_REQUEST_GEAR_DATA, new Object[] { id }));
			...
			{// motion on planets
				if (!player.onGround && player.worldObj.provider instanceof IGalacticraftWorldProvider && player.ridingEntity == null && !(player.inventory.getCurrentItem() != null && player.inventory.getCurrentItem().getItem() instanceof IHoldableItem)) {
	...
	private void hurt(EntityPlayer player, float ticks)
	{
		if(player.hurtTime > 0 || player.deathTime > 0)
		{
			...
			GL11.glColor4f(player.getBrightness(ticks), 0.0F, 0.0F, 1.0F);
